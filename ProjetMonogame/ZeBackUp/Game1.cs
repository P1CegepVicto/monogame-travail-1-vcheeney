...
namespace Projet1
{
    ...
    public class Game1 : Game
    {
        ...
        protected override void LoadContent()
        {
            ...
            bullethero = new GameObject((int)hero.position.X, (int)hero.position.Y, 10, 10, false); //ici on a pas le Health, il est facultatif.
            bullethero.sprite = Content.Load<Texture2D>("Bullets\\BULLET1B.png");
            bullethero.origine.X = bullethero.sprite.Width / 2;
            bullethero.origine.Y = bullethero.sprite.Height / 2;
            bullethero.vitesse = 10;
            ...
            bulletenemy.origine.X = bullethero.sprite.Width / 2;
            bulletenemy.origine.Y = bullethero.sprite.Height / 2;
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            spriteBatch.Draw(bullethero.sprite, bullethero.position, null, Color.White, bullethero.angle, bullethero.origine, 1f, SpriteEffects.None, 0f);
        ...
        public void UpdateHero() ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////UPDATE HERO
        {
            ...
            if (hero.health <= 0)
            {
                ...
                bullethero.position.X = fenetre.Width;
                bullethero.position.Y = fenetre.Height;
            }
        ...
        public void UpdateBulletHero() //////////////////////////////////////////////Bullet hero;
        {
            ...
            {
                bullethero.position.X = fenetre.Width + 200;
                bullethero.position.Y = fenetre.Height + 200;
            }
            ...
            if (hero.estVivant == true) //IF !!!HERO!!! EST VIVANT;
            {
                if (shoothero == 0)
                {
                    ...
                    bullethero.position.X = hero.position.X + ((hero.sprite.Width / 2));
                    bullethero.position.Y = hero.position.Y + ((hero.sprite.Height / 2));

                    double grandissement1;

                    distanceHVx = bullethero.position.X - viseur.position.X - ((viseur.sprite.Width) / 2);
                    distanceHVy = bullethero.position.Y - viseur.position.Y - ((viseur.sprite.Height) / 2);
                    distanceHV = Math.Sqrt(Math.Pow(distanceHVx, 2) + Math.Pow(distanceHVy, 2));

                    grandissement1 = (distanceHV / bullethero.vitesse);

                    bullethero.direction.X = -(float)(distanceHVx / grandissement1);
                    bullethero.direction.Y = -(float)(distanceHVy / grandissement1);

                    shoothero = 1;
                }

                bullethero.position += bullethero.direction;
                bullethero.angle -= (float)(Math.PI / 15);
            }

            //bullethero.angle +=(float)(Math.PI/30);

            //On kill le "bullet" si il sort de la zone de jeu.
            if (bullethero.position.Y > fenetre.Height || bullethero.position.Y < 0 || bullethero.position.X > fenetre.Width || bullethero.position.X < 0)
            {
                bullethero.estVivant = false;
            }
        }
        ...
        public void UpdateColisions() /////////////////////////////////////////////////////////////////////////////////////////////////////Update Collisions
        {
            ...
            if (enemy.hitbox(35).Intersects(bullethero.hitbox(6)) && bullethero.estVivant == true)
            {
                enemy.health = enemy.health - 10;
                bullethero.estVivant = false;
            }
            if (bulletenemy.hitbox(6).Intersects(bullethero.hitbox(6)))
            {
                bulletenemy.estVivant = false;
                bullethero.estVivant = false;
            }
